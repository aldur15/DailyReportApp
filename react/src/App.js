...
const ReportForm = ({ token, onReportSubmitted }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        console.log('Report submitted!'); // TODO: Replace with user-friendly UI feedback
        setFormData({ title: '', summary: '', date: new Date().toISOString().split('T')[0] });
        onReportSubmitted();
      } else {
        const data = await res.json();
        console.log('Error submitting report: ' + (data.detail || JSON.stringify(data))); // TODO: Replace with user-friendly UI feedback));
      }
    } catch (err) {
      console.log('Network error: ' + err.message); // TODO: Replace with user-friendly UI feedback
    } finally {
...
const ReportItem = ({ report, token, onUpdate, onDelete }) => {
  ...
  const handleEdit = () => {
    ...
    fetch(`${API_BASE}/reports/${report.id}`, {
      ...
      .then(data => {
        if (data.detail) {
          console.log('Error updating report: ' + JSON.stringify(data)); // TODO: Replace with user-friendly UI feedback);
        } else {
          console.log('Report updated'); // TODO: Replace with user-friendly UI feedback
          onUpdate();
        }
      });
  };
  ...
  const handleDelete = () => {
    ...
    fetch(`${API_BASE}/reports/${report.id}`, {
      ...
      .then(res => {
        if (res.ok) {
          console.log('Report deleted.'); // TODO: Replace with user-friendly UI feedback
          onDelete();
        } else {
          return res.json().then(data => {
            console.log('Error deleting report: ' + (data.detail || JSON.stringify(data))); // TODO: Replace with user-friendly UI feedback));
          });
        }
      })
      .catch(err => console.log('Request failed: ' + err)); // TODO: Replace with user-friendly UI feedback);
  };
  ...
  const checkUser = async () => {
    try {
      ...
      if (res.ok) {
        ...
        console.log('Session expired'); // TODO: Replace with user-friendly UI feedback
        onLogout();
      }
    } catch (err) {
      console.log('Network error: ' + err.message); // TODO: Replace with user-friendly UI feedback
